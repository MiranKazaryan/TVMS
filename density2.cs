//Данный моудль представляет дочерний класс, генерирует задание по квадратичной плотности распределения
...
namespace Generators
{
    ...
    class density2 : base_density
    {
        private static density2 instance;
        public double k1;
        public double k2;
        public double b1;
        public double b2;
        ...
        public static density2 Instance
        {
            get
            {
                if (instance == null)
                {
                    instance = new density2();
                }
                return instance;
            }
        }
        ...
        public string task(int i)
        {
            ...
            // sokr_dr((int)k1, (int)k2);
            k1 = k1 / sokr_dr((int)k1, (int)k2);
            k2 = k2 / sokr_dr((int)k1, (int)k2);
            //kx+b,x(a,b)
            //sokr_dr(-2, ((int)Math.Pow((alpha - beta), 2)));
            if (k2 == 1)
                s += "<m:mn>" + k1 + "</m:mn>";
            else
                s += "<m:mfrac><m:mn>" + k1 + "</m:mn><m:mn>" + k2 + "</m:mn></m:mfrac>";
            //sokr_dr(-2 * alpha, ((int)Math.Pow((alpha - beta), 2)));
            //<msup><mi>x</mi><mn>2</mn></msup>
            s += "<m:mo>(</m:mo>";
            s += "<m:msup><m:mi>x</m:mi><m:mn>2</m:mn></m:msup>";
            s += "<m:mo>-</m:mo>";
            s += "<m:mn>" + (alpha + beta) + "</m:mn>";
            s += "<m:mi>x</m:mi>";
            s += "<m:mo>+</m:mo>";
            s += "<m:mn>" + (alpha * beta) + "</m:mn>";
            s += "<m:mo>)</m:mo>";
        ...
        {
            Mx = (Math.Round((((double)alpha + (double)beta) / 2), 3));
            return Mx;
        }
        //Дисперсия
        public double answer5()
        {
            Dx = (Math.Round((Math.Pow((beta - alpha), 2) / 3), 3));
            return Dx;
        }
